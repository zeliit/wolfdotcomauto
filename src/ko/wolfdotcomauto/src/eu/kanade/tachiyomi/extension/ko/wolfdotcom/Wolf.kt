...
open class Wolf(
    ...
    override val name = "늑대닷컴 Auto - $name"
    ...
    private fun domainNumberInterceptor(chain: Interceptor.Chain): Response {
        ...
        extractDomainNumberFromPopup(response)?.let { newDomainNum ->
            if (newDomainNum != domainNumber) {
                response.close()
                domainNumber = newDomainNum
                return chain.proceed(rebuildRequestWithCurrentDomain(initialRequest))
            }
        }

        if (!response.isSuccessful) {
            val previousDomainNum = domainNumber
            refreshDomainNumberFromTelegram(force = true)

            if (domainNumber != previousDomainNum) {
                response.close()
                response = chain.proceed(rebuildRequestWithCurrentDomain(initialRequest))
            }
        }
    ...
    private fun rebuildRequestWithCurrentDomain(request: Request): Request {
    ...
    private fun refreshDomainNumberFromTelegram(force: Boolean = false) {
        val now = System.currentTimeMillis()
        val lastCheckedAt = preference.getLong(PREF_TELEGRAM_LAST_CHECK_AT, 0L)

        if (!force && now - lastCheckedAt < TELEGRAM_CHECK_INTERVAL_MS) return

        preference.edit().putLong(PREF_TELEGRAM_LAST_CHECK_AT, now).apply()

        val newDomainNum = fetchDomainNumberFromTelegram() ?: return
        if (newDomainNum != domainNumber) {
            domainNumber = newDomainNum
        }
    }

    private fun fetchDomainNumberFromTelegram(): String? = runCatching {
        telegramClient.newCall(GET(TELEGRAM_CHANNEL_URL, headers)).execute().use { response ->
            if (!response.isSuccessful) {
                ...
                val hrefCandidates = document.select("a[href]").eachAttr("href")
                val textCandidate = document.text()

                hrefCandidates.asSequence()
                    .mapNotNull(::extractDomainNumber)
                    .firstOrNull()
                    ?: extractDomainNumber(textCandidate)
            }
        }
    }.getOrNull()

    private fun extractDomainNumberFromPopup(response: Response): String? {
        val document = Jsoup.parse(response.peekBody(Long.MAX_VALUE).string())
        val hrefCandidates = document.select("#pop-content a[href]").eachAttr("href")

        return hrefCandidates.asSequence()
            .mapNotNull(::extractDomainNumber)
            .firstOrNull()
    }
